  ...
  private generateHTMLContent(data: StudentTranscriptData): string {
    ...
    const courseRows = data.courses
      .map(
        (course, index) => `
      <tr>
        <td style="text-align: center; padding: 4px 8px; border-bottom: 1px solid #000;">${
          index + 1
        }</td>
        <td style="text-align: center; padding: 4px 8px; border-bottom: 1px solid #000;">${
          course.code
        }</td>
        <td style="padding: 4px 8px; border-bottom: 1px solid #000;">${
          course.name
        }</td>
        <td style="text-align: center; padding: 4px 8px; border-bottom: 1px solid #000;">${
          course.credits
...
<body>
  ...
  private async htmlToPDF(data: StudentTranscriptData): Promise<Uint8Array> {
    ...
    // Summary section
    currentY = addCenteredText(
      `Total Jumlah SKS = ${data.totalCredits}`,
      currentY,
      11,
      "normal"
    );
    ...
    const cleanSignature = data.digitalSignature
      .replace(/\r?\n|\r/g, "")
      .replace(/\s+/g, "");
    ...
    const charWidth =
      (doc.getStringUnitWidth("X") * 7) / doc.internal.scaleFactor;
    const maxCharsPerLine = Math.floor(availableWidth / charWidth) - 2; // Leave some buffer

    const signatureLines = this.wrapText(
